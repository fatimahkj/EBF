...
'''
def reachabilitycheck(filename):
...
def main(args):
	...
	#
	if inputfile != '':    # test a specific file only
		sortedfiles = []
		if reachabilitycheck(inputfile.replace('.i','.yml')): sortedfiles.append(inputfile)
	elif inputpath != '':  # test all files at the given path
		# Spawn parallel processes . .   .
		#
		filteredlist = []

		for f in listfiles(inputpath):
			if reachabilitycheck(f.replace('.i','.yml')):
				print ("adding [%s]" % f)
				filteredlist.append(f)
			else: print ("skipping [%s]" % f)
