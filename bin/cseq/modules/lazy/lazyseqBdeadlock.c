...
int pthread_mutex_lock(cspthread_mutex_t *__cs_mutex_to_lock) {
	...
	if (*__cs_mutex_to_lock==-1) {
		*__cs_mutex_to_lock = __cs_thread_index+1;
		...
		__VERIFIER_assertext(!(__cs_chaincounter>1 && __cs_chain[0]==__cs_thread_index+1 && *__cs_mutex_to_lock==__cs_chain[__cs_chaincounter-1]), "deadlock detected");
...
int pthread_mutex_unlock(cspthread_mutex_t *__cs_mutex_to_unlock) {
	...
	__VERIFIER_assertext(*__cs_mutex_to_unlock==__cs_thread_index+1,"attempt to release a lock held by another thread");

	if (__cs_chaincounter>1) __VERIFIER_assume(__cs_mutex_to_unlock!=__cs_secondlock);
