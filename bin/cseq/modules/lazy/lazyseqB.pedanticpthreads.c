...
__cs_key_t __cs_keys[1][THREADS+2];
...
int __cs_self(void) { return __cs_thread_index+1; }
...
int __cs_mutex_lock(__cs_mutex_t *__cs_mutex_to_lock) {
	__CSEQ_assume(*__cs_mutex_to_lock==-1 || *__cs_mutex_to_lock==0);
...
int __cs_cond_wait_2(__cs_cond_t *__cs_cond_to_wait_for, __cs_mutex_t *__cs_m) {
	_Bool b;
	if (b) __CSEQ_assume(*__cs_cond_to_wait_for == 1);
...
int __cs_barrier_wait_2(__cs_barrier_t *__cs_barrier_to_wait){
	__CSEQ_assertext(__cs_barrier_to_wait->init > 0, "attempt to use an uninitialized barrier variable");
	_Bool b;
	if (b) __CSEQ_assume(__cs_barrier_to_wait->current == 0);
	__cs_barrier_to_wait->current = __cs_barrier_to_wait->init;
	return 0;
}

__cs_key_create(__cs_key_t *key, void (*destructor)(void*)) {
	static int currentkey = 0;
	__CSEQ_assertext(currentkey < 1, "multiple thread-specific keys not supported");
	__cs_key_destructor[0] = destructor;
	*key = currentkey++;
	return 0;
}

int __cs_setspecific(__cs_key_t key, const void *value) {
